...
$(document).ready(function () {
	...
	if ($(window).width() > 991) {
		...
		/* App Design 2*/
		var $app2DesignTrigger = $('.app-design2'),
			$app2DesignVisual = $('.app-design__visual2');

		var app2DesignScene = new ScrollMagic.Scene({
			triggerElement: $app2DesignTrigger,
			reverse: false
		})
			.setTween(TweenLite.from($app2DesignVisual, 0.8, { x: 100, opacity: 0, ease: Power1.easeOut }))
			.addTo(controller);
		...
		/* Work 2*/
		var $workTrigger2 = $('.work2'),
			$workContent2 = $('.work__content2'),
			$workVisual2 = $('.work__visual2'),
			$workFrame2 = CSSRulePlugin.getRule('.work__list2:before'),
			workTl = new TimelineLite();

		workTl
			.set($workVisual2, { scale: 1 })
			.from($workContent2, 0.8, { x: -50, opacity: 0, ease: Power1.easeOut })
			.from($workVisual2, 0.8, { x: 50, opacity: 0, ease: Power1.easeOut }, 0)
			.from($workFrame2, 0.8, { cssRule: { opacity: 0 }, ease: Power1.easeOut });

		var workScene2 = new ScrollMagic.Scene({
			triggerElement: $workTrigger2,
			offset: 60,
			reverse: false
		})
			.setTween(workTl)
